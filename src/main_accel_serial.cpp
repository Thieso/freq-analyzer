...
void setup(void)
{
    ...
    // initialize the push button for logging control with a pullup resistor
    pinMode(BUTTON_PIN, INPUT_PULLUP);
    digitalWrite(BUTTON_PIN, HIGH);

    // initialize the logging LED
...
void loop(void)
{
    // Wait for button to be pushed to start logging
    ...
    delay(1000);
    // logging started so turn LED on
    digitalWrite(LOGGING_LED, HIGH);

    // display stopping message
    Serial.println("Started Logging: Press Button to stop logging");
    ...
    // convert to csv file after logging is finished
    binaryToCsv();
